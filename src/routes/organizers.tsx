  ...
  return (
    <div className="min-h-screen bg-background pt-24 pb-16">
      <div className="max-w-7xl mx-auto px-5 sm:px-6 w-full text-left">
        {/* Header Section */}
        <div className="mb-8">
          <div className="inline-flex items-center gap-1.5 bg-primary/10 text-primary border border-primary/20 text-[10px] font-black uppercase tracking-widest px-2.5 py-0.5 rounded-[4px] mb-3">
            {t("சங்க நிர்வாகிகள்", "TNVS ORGANIZERS")}
          </div>
          <h1 className="font-display text-3xl font-extrabold text-ink tracking-tight leading-none">
          ...
          <form onSubmit={handleSearchSubmit} className="flex flex-col md:flex-row gap-4 items-end">
            <div className="flex-1 w-full space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-muted border border-border rounded-md pl-12 pr-4 py-2.5 text-sm text-foreground placeholder-slate-400 focus:outline-none focus:bg-card focus:border-primary transition-all"
                />
              </div>
            </div>
            ...
            <div className="w-full md:w-48 space-y-1.5">
              ...
              <input
                ...
                className="w-full bg-muted border border-border rounded-md px-4 py-2.5 text-sm text-foreground placeholder-slate-400 focus:outline-none focus:bg-card focus:border-primary transition-all"
              />
            </div>
            ...
            <div className="flex gap-2 w-full md:w-auto shrink-0">
              <button
                type="submit"
                className="flex-1 md:flex-none bg-primary hover:bg-[#007a38] text-white font-bold py-2.5 px-6 rounded-md text-sm transition cursor-pointer"
              >
                {t("தேடுக", "Filter")}
              </button>
              {(search || district || assembly) && (
                <button
                  ...
                  className="bg-muted hover:bg-muted text-slate-605 font-bold py-2.5 px-4 rounded-md text-sm transition cursor-pointer border border-border"
                >
                  {t("ரத்து", "Reset")}
                </button>
              )}
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-rose-500 bg-rose-50 rounded-md border border-rose-200 p-6">
            <AlertCircle className="w-8 h-8 mb-3" />
            <p className="text-sm font-bold">{t("பிழை ஏற்பட்டது", "An error occurred")}</p>
            <p className="text-xs text-rose-600 mt-1">{error}</p>
          </div>
        ) : organizers.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-muted-foreground bg-card rounded-md border border-border p-6 text-center">
            <Shield className="w-12 h-12 text-muted-foreground mb-3" />
            <h3 className="text-base font-bold text-foreground">{t("நிர்வாகிகள் யாரும் இல்லை", "No Organizers Found")}</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {organizers.map((org) => (
              <div
                key={org.id}
                className="bg-card border border-border hover:border-primary/30 rounded-md p-6 hover:shadow-xs transition duration-200 flex flex-col justify-between"
              >
                <div className="space-y-4">
                  <div className="flex items-center gap-3.5">
                    <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-amber-505 bg-primary/10 flex items-center justify-center text-amber-600 font-bold shrink-0">
                      <Shield className="w-6 h-6 text-primary" />
                    </div>
                    <div className="min-w-0 text-left">
                      <h3 className="font-bold text-foreground text-sm truncate">{org.name}</h3>
                  ...
                  <div className="space-y-2 border-t border-border pt-3.5 text-xs text-left">
                    ...
                    {org.email && (
                      <div className="flex justify-between items-center gap-2">
                        <span className="text-muted-foreground shrink-0">{t("மின்னஞ்சல்", "Email")}</span>
                    ...
                    {(org.assembly || org.district) && (
                      <div className="flex justify-between items-start gap-2">
                        <span className="text-muted-foreground shrink-0">{t("வட்டாரம்", "Location")}</span>
                        <span className="font-bold text-foreground flex items-start gap-1">
